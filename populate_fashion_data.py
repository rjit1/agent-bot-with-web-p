#!/usr/bin/env python3
"""
Phase 2: Populate Size and Color Data for Fashion Products
This script updates all products with appropriate sizes and colors based on their categories.
"""
...
def populate_product_data():
    """Populate size and color data for all products."""
    ...
    print('🚀 Starting Fashion Product Data Population...')
    ...
    try:
        # Get all products
        result = supabase.table('products').select('id, product_id, title, category, colors, specifications').execute()
        ...
        for product in result.data:
            ...
            # Update specifications with sizes
        ...
        # Verify the updates
        print('\n🔍 Verifying updates...')
        verify_result = supabase.table('products').select('product_id, title, category, size_range, colors, specifications').execute()
        ...
        if verify_result.data:
            ...
            for i, product in enumerate(verify_result.data[:5]):
                ...
                print(f'   Sizes: {product.get("size_range", "N/A")}')
